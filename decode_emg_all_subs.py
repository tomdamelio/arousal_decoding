...
for subject in subjects:
    ...
    plt.savefig(op.join(derivative_path, 'sub-' + subject , 'eeg','sub-' + subject +
                f'_plot_NO_OPT_PIPELINE_models_DEAP_emg_{subject}_{cv_name}.png'))

    scores = cross_val_score(X=X, y=y, estimator=clf, cv=cv, scoring='r2')
    np.save(op.join(derivative_path, 'sub-' + subject , 'eeg','sub-' + subject +
                f'_scores_NO_OPT_PIPELINE_models_DEAP_emg_{subject}_{cv_name}.npy'),
                scores)
# %%
subject = '01'
x = np.load(op.join(derivative_path, 'sub-' + subject , 'eeg','sub-' + subject +
                    '_scores_NO_OPT_PIPELINE_models_DEAP_emg_01_2Fold.npy'), allow_pickle=True)
