...
fname_data <- str_glue("./outputs/DEAP-bids/derivatives/mne-bids-pipeline/emg-scores-drago/sub-{sub}_all_scores_models_DEAP_emg_r2_2Fold.npy")
data <- np$load(
  fname_data,
  allow_pickle = T)[[1]] %>%
  as.data.frame()
...
# CONCATENAR ACA CON MAS DATASETS
...
fname <- str_glue("./outputs/DEAP-bids/derivatives/mne-bids-pipeline/emg-scores-drago/fig_DEAP_emg_model_comp")
