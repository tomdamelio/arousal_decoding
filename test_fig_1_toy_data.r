...
data <- np$load(
  "./outputs/DEAP-bids/derivatives/mne-bids-pipeline/sub-02/eeg/sub-02_all_scores_models_DEAP_emg_r2_2Fold.npy",
  allow_pickle = T)[[1]] %>%
  as.data.frame()

# Create a toy df of 'data'
data <- as.data.frame(matrix(rnorm(7*10, mean=0, sd=0.3), ncol=7))  
